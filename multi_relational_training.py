...
class Experiment:
    ...
    def __init__(self, learning_rate=50, dim=40, nneg=50, model="poincare",
                 num_iterations=500, batch_size=128, dataset="wiktionary", mps=False):
    ...
    def train_and_eval(self):
        ...
        if self.model_type == "poincare":
            model = torch.jit.script(MuRP(d, self.dim)).to(device)
        else:
            model = torch.jit.script(MuRE(d, self.dim)).to(device)
