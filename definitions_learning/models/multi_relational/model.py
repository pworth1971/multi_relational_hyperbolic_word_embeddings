...
class MuRP(torch.nn.Module):
    ...
    def one_shot_encoding(self, v_idx, r_idx):
        #Export function for torch.jit.script
        v = self.Eh.weight[v_idx]
        Ru = self.Wu[r_idx]
        rvh = self.rvh.weight[r_idx]
        ...
        # We want u_m to be as close as possible to v after applying a relation-specific transformation (v_m). 
        # Therefore, to derive u, we perform an inverse transformation from v_m via the Ru matrix.
        v_m = p_sum(torch.mean(v,1), torch.mean(rvh,1))

        # return the average u vector from all the terms in the definition
        return v_m
