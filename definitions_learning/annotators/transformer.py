    ...
    def __init__(self, annot_model = None):
        ...
        if not annot_model:
            if (Annotator.default_model):
                ...
                self.annot_model = AutoModelForTokenClassification.from_pretrained(settings["transformer_annotator_model"]).to(self.device)
                self.annot_tokenizer = AutoTokenizer.from_pretrained(settings["transformer_annotator_model"])
                self.ids_to_labels = json.loads(settings["transformer_annotator_labels"])
                self.MAX_LEN = settings["transformer_annotator_max_len"]
